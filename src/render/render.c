...
void	draw_gradient(void)
{
	...
	for (uint32_t j = 0; j < image->height; j++)
	{
		...
		{
			c1 = 1.0 * i / image->width;
			c2 = 1.0 * j / image->height;
...
void	compute_viewport(t_data *data)
{
	...
	pcam = &data->cam;
	viewport_w = tan(data->cam.angle * M_PI / 360) * 1; // distance between the viewer and the field of view = 1.
...
t_ray	ft_compute_ray(t_cam cam, int row, int col)
{
	...
	pixel_pos = ft_vec3_add(
			ft_vec3_add(ft_vec3_scal_prod(cam.du, col),
